...
# check head
execute store result score .new tungsten.uid.head run attribute @s minecraft:generic.luck modifier value get c-0-a455-0-c 1000000000000
execute unless score .new tungsten.uid.head = @s tungsten.uid.head run function tungsten:swap/head
# check chest
execute store result score .new tungsten.uid.chest run attribute @s minecraft:generic.luck modifier value get c-0-a455-0-d 1000000000000
execute unless score .new tungsten.uid.chest = @s tungsten.uid.chest run function tungsten:swap/chest
# check legs
execute store result score .new tungsten.uid.legs run attribute @s minecraft:generic.luck modifier value get c-0-a455-0-e 1000000000000
execute unless score .new tungsten.uid.legs = @s tungsten.uid.legs run function tungsten:swap/legs
# check feet
execute store result score .new tungsten.uid.feet run attribute @s minecraft:generic.luck modifier value get c-0-a455-0-f 1000000000000
